...
const PIUTANG_PAGE_CACHE = "__piutangPageCache";
...
// Snapshot data milik instance ini. Store per jenis laporan Piutang
// ("kartupiutang", "sisapiutang", dst.) memang sudah terpisah satu sama
// lain, tapi snapshot lokal tetap dipakai supaya tampilan tidak "reset" ke
// popup filter saat tab-nya sempat tercoret dari cache keep-alive (mis.
// karena jumlah tab terbuka banyak) lalu dibuka lagi.
const localData = ref([]);
...
function savePageSnapshot() {
  ...
  window[PIUTANG_PAGE_CACHE] = window[PIUTANG_PAGE_CACHE] || {};
  window[PIUTANG_PAGE_CACHE][props.storeModule] = {
...
window[PIUTANG_PAGE_CACHE] = window[PIUTANG_PAGE_CACHE] || {};
const cachedPage = window[PIUTANG_PAGE_CACHE][props.storeModule];
if (cachedPage) {
...
onUnmounted(() => {
  ...
  if (!isTabStillOpen && window[PIUTANG_PAGE_CACHE]) {
    delete window[PIUTANG_PAGE_CACHE][props.storeModule];
  }
});
</script>
